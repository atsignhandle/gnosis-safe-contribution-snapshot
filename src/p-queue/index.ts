...
*/
class PQueue<QueueType 
extends Queue<RunFunction, EnqueueOptionsType> = PriorityQueue, EnqueueOptionsType 
extends QueueAddOptions = DefaultAddOptions> 
extends EventEmitter<'active' | 'idle' | 'completed' | 'error' | 'next'> {
	...
	constructor(options?: Options<QueueType, EnqueueOptionsType>) {
		...
		if (!(typeof options.intervalCap === 'number' && options.intervalCap >= 1)) {
			throw new TypeError
			(`Expected \`intervalCap\` to be a number from 1 and up, got \`${options.intervalCap?.toString() ?? ''}\` 
    ...
    */
    async onSizeLessThan(limit: number) {
        // Instantly resolve if the queue is empty.
...
export {PQueue, Queue, QueueAddOptions, DefaultAddOptions, Options};
